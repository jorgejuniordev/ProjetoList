...
<!-- content area -->    
    <div class="row"> 
        <div class="grid_5">
            <h2 class="first-header"><center>PCI</center></h2>
            <img src="img/pci.jpg" />
            <form method="GET" action="?action=pci">
                <input type="hidden" name="action" value="pci">
                <?php if(isset($_GET['led07']) && ($_GET['led07']=='on')){ ?>
                    <button type="submit" value="off" class="buttonlink" name="led07">Desligar</button>
                <?php }else{ ?>
                    <button type="submit" value="on" class="buttonlink" name="led07">Ligar</button>
                <?php } ?>
            </form>
        </div>
        <div class="grid_7">
