...
<!-- content area -->    
    <div class="row"> 
        <div class="grid_5">
            <h2 class="first-header"><center>FONTE ALIMENTAÇÃO</center></h2>
            <img src="img/fal.jpg" />
            <form method="GET" action="?action=fal">
                <input type="hidden" name="action" value="fal">
                <?php if(isset($_GET['led02']) && ($_GET['led02']=='on')){ ?>
                    <button type="submit" value="off" class="buttonlink" name="led02">Desligar</button>
                <?php }else{ ?>
                    <button type="submit" value="on" class="buttonlink" name="led02">Ligar</button>
                <?php } ?>
            </form>
        </div>
        <div class="grid_7">
