...
<!-- content area -->    
    <div class="row"> 
        <div class="grid_5">
            <h2 class="first-header"><center>Memória RAM</center></h2>
            <img src="img/mpr.jpg" />
            <form method="GET" action="?action=mpr">
                <input type="hidden" name="action" value="mpr">
                <?php if(isset($_GET['led05']) && ($_GET['led05']=='on')){ ?>
                    <button type="submit" value="off" class="buttonlink" name="led05">Desligar</button>
                <?php }else{ ?>
                    <button type="submit" value="on" class="buttonlink" name="led05">Ligar</button>
                <?php } ?>
            </form>
        </div>
        <div class="grid_7">
