...
<!-- content area -->    
    <div class="row"> 
        <div class="grid_5">
            <h2 class="first-header"><center>Memória DVD</center></h2>
            <img src="img/mdr.jpg" />
            <form method="GET" action="?action=mdr">
                <input type="hidden" name="action" value="mdr">
                <?php if(isset($_GET['led06']) && ($_GET['led06']=='on')){ ?>
                    <button type="submit" value="off" class="buttonlink" name="led06">Desligar</button>
                <?php }else{ ?>
                    <button type="submit" value="on" class="buttonlink" name="led06">Ligar</button>
                <?php } ?>
            </form>
        </div>
        <div class="grid_7">
